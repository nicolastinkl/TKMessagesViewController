                   ...
                   avatar:(BOOL)hasAvatar
{
    ...
    switch ([message.messageType intValue]) {
        ...
        {
            JSBubbleImageView *bubbleView = [[JSBubbleImageView alloc] initWithFrame:frame
                                                                bubbleType:type
                                                           bubbleImageView:bubbleImageView];
...
//    self.bubbleImageView.imageView.imageView.image = nil;
...
- (void)setBackgroundColor:(UIColor *)color
{
    ...
    [self.bubbleImageView setBackgroundColor:color];
}
...
- (void)setMessage:(FCMessage*)message
{
    ...
    switch ([message.messageType intValue]) {
        ...
        case messageType_image:
        {
            [self setImageViewImg:[message imageUrl]];
        }
            break;
    ...
    [self setTimestamp:[message sentDate]];
    [self setSubtitle:[message sender]];
}

-(void) setImageViewImg:(NSString *) url
{
    [self.bubbleImageView.imageView setImage:[UIImage imageNamed:@"imagetwo"] forState:UIControlStateNormal];
}
...
{
    return [self.currentMessage.messageStatus boolValue]?JSBubbleMessageTypeIncoming:JSBubbleMessageTypeOutgoing;
//    return _bubbleView.type;
}

#pragma mark - Class methods

+ (CGFloat)neededHeightForBubbleMessageCellWithMessage:(FCMessage*)message
                                                avatar:(BOOL)hasAvatar
{
    CGFloat timestampHeight = [message sentDate] ? kJSTimeStampLabelHeight : 0.0f;
    ...
    switch ([message.messageType intValue]) {
        ...
        case messageType_image:
        {
            CGFloat bubbleHeight = [JSBubbleImageView neededHeightForImageview];
            return subviewHeights + MAX(avatarHeight, bubbleHeight);
        }
            break;
...
{
    if ([self.currentMessage.messageType intValue] == messageType_text) {
        
        return (action == @selector(copy:) || action == @selector(deleteclick:) || action == @selector(directclick:));
    
    }
    
    return ( action == @selector(deleteclick:) || action == @selector(directclick:));
...
- (void)handleLongPressGesture:(UILongPressGestureRecognizer *)longPress
{
    ...
    UIView * fromview;
    if (self.bubbleView) {
        viewframe = [self.bubbleView bubbleFrame];
        fromview = self.bubbleView;
    } else if(self.bubbleImageView)
    {
        viewframe = [self.bubbleImageView bubbleFrame];
        fromview = self.bubbleImageView;
    }
    CGRect targetRect = [self convertRect:viewframe
                                 fromView:fromview];
    UIMenuItem * itemDelete = [[UIMenuItem alloc] initWithTitle:@"删除" action:@selector(deleteclick:)];
    UIMenuItem * itemDirect = [[UIMenuItem alloc] initWithTitle:@"转发" action:@selector(directclick:)];
    menu.menuItems = @[itemDirect,itemDelete];
    ...
    {
        self.bubbleImageView.bubbleImageView.highlighted = YES;
    }
...
- (void)handleMenuWillHideNotification:(NSNotification *)notification
{
    ...
    {
        self.bubbleImageView.bubbleImageView.highlighted = NO;
    }
